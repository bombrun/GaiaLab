...
# Setting the last value of differences_lower==0 ensures it will always 
# return a false positive. Rectify that here. In reality you cannot 
...
def filter_turning_points(df, threshold=1):
    ...
    turning_points = [True if abs(diff) > threshold else False for diff in \
                     [0,*np.diff(working_df['rate'] - working_df['w1_rate'])]]
...
def response_time(df, t=10, window_size=25):
    ...
    window = (working_df['rate'] - working_df['w1_rate'])[working_df['obmt'] \
