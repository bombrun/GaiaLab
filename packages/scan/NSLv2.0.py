...
class Source:
    ...
    def update(self, t):
        self.alpha = self.alpha + self.mu_alpha_dx*t
        self.delta = self.delta + self.mu_delta*t
    ...
    def u_xyz(self, satellite, t):
        self.update(t)
        p, q, r = ft.pqr(self.alpha, self.delta)
        u = self.u_bcrs(t) + t*(p*self.mu_alpha_dx + q*self.mu_delta + r*self.mu_radial)
